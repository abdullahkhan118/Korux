...
class JwtService<T: JwtAuthenticatedUser<ID>,ID,R: AuthenticateUserRepository<T,ID>>(
    @PropertyKey() override val secretKey: String,
    override val userDetailsService: UserDetailsService,
    override val repository: R
) : AuthService<T, ID,R> {

    override fun generateToken(user: UserDetails, expirationTime: Long?): String = Jwts
        .builder()
    ...
    override fun isTokenValid(token: String, user: UserDetails) =
    ...
    override fun extractUsername(token: String): String = extractClaims(token) {
        it.subject
    }

    override fun extractExpirationDate(token: String): Date = extractClaims(token) {
        it.expiration
    }

    override fun isExpired(token: String) = extractExpirationDate(token).before(Date())
    ...
    override fun signingKey(): SecretKey = Keys.hmacShaKeyFor(Decoders.BASE64.decode(secretKey))
}